...
def _safe_boot_identity() -> BootIdentity:
    # kern.boottime is available on macOS; Linux/CI receives a deterministic
    # per-boot approximation without making the protocol depend on a platform.
    try:
...
class StorageBrokerClient:
    ...
    def _run_locked(self, lock_set: Any, request: _StorageBrokerRequest, *, effect_budget_ns: int, cleanup_budget_ns: int) -> StorageWorkflowRecord:
        ...
        if self.transport is None:
            self.ledger.close_preexec_failure_locked(lock_set, record.workflow_id, record.generation)
            recovery.close()
    ...
    def _run_probe_record(self, lock_set: Any, request: _BrokerProbeRequest, effect_budget_ns: int) -> StorageWorkflowRecord:
        ...
        recovery = _new_recovery_authority(self.home)
        try:
            return self.ledger.prepare_locked(lock_set, request, self.executable, _socket_identity(self.home), _safe_boot_identity(), recovery, effect_budget_ns=effect_budget_ns, cleanup_budget_ns=0)
        finally:
            recovery.close()
