...
describe("StatusRail", () => {
  it("keeps transport and executor truth independent and humanized", () => {
    render(<StatusRail transport="connected" executor="unavailable" latencyMs={null} />);
    expect(screen.getByTitle("Statut de la connexion ChatGPT")).toHaveTextContent("Connecté");
    expect(screen.getByTitle("Statut de l'agent exécutif local")).toHaveTextContent("Indisponible");
